...
class TenantService {
  async getTenantIdBySubdomain(subdomain: string): Promise<string | null> {
    const tenantIdFromCache = await RedisService.getClient().get(
      `${REDIS_TENANT_SUBDOMAIN_PREFIX}${subdomain}`
    );
  ...
  async isTenantInitialized(tenantId: string): Promise<boolean> {
    const isTenantInitialized = await RedisService.getClient().get(
      `${REDIS_TENANT_INIT_FLAG_PREFIX}${tenantId}`
    );
  ...
  async setTenantInitialized(tenantId: string): Promise<void> {
    await RedisService.getClient().set(`${REDIS_TENANT_INIT_FLAG_PREFIX}${tenantId}`, 'true', {
      EX: 3600,
    });
  }

  async getTenantBySubdomain(subdomain: string): Promise<TenantDocument | null> {
