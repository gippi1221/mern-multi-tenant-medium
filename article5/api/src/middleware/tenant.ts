...
import { bookSchema, refreshTokenSchema, userScema } from '../models';
...
export const setupTenant = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!db.models['User']) {
      db.model('User', userScema);
      console.log('User model applied to a new connection');
    }

    if (!db.models['RefreshToken']) {
      db.model('RefreshToken', refreshTokenSchema);
      console.log('RefreshToken model applied to a new connection');
    }
